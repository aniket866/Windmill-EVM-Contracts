...
import {
    ZeroAddress, SameToken, ZeroAmount, ZeroStartPrice,
    InvalidExpiry, InvalidPriceBounds, SlopeOverflow,
    NotMaker, OrderInactive, OrderExpired,
    SelfMatch, NoCross, PairMismatch
...
contract WindmillExchangeTest is Test {
    ...
    {
        vm.prank(maker);
        return exchange.createOrder(address(tokenA), address(tokenB), amt, price, slope, 0, 0, expiry, true);
    }
    ...
    {
        vm.prank(maker);
        return exchange.createOrder(address(tokenB), address(tokenA), amt, price, slope, 0, 0, expiry, false);
    }
    ...
    function test_matchOrders_fullFill() public {
        ...
        // Pair index cleared
        assertEq(exchange.getOrdersByPair(address(tokenA), address(tokenB), 0, type(uint256).max).length, 0);
    }
    ...
    function test_matchOrders_revert_selfMatch() public {
        ...
        uint256 buyId = _buy(alice, 100 ether, RAY, 0, 0);
        vm.prank(alice);
        uint256 sellId = exchange.createOrder(address(tokenB), address(tokenA), 100 ether, RAY, 0, 0, 0, 0, false);
    ...
    function test_matchOrders_revert_pairMismatch() public {
        ...
        vm.prank(bob); tokenA.approve(address(exchange), type(uint256).max);
        vm.prank(bob);
        uint256 wrongId = exchange.createOrder(address(tokenA), address(tokenB), 100 ether, RAY, 0, 0, 0, 0, true);
    ...
    function test_pruneExpiredOrders() public {
        ...
        assertEq(tokenA.balanceOf(alice), aliceBefore - 100 ether, "expired escrow refunded only for id1");

        uint256[] memory remaining = exchange.getOrdersByPair(address(tokenA), address(tokenB), 0, 10);
